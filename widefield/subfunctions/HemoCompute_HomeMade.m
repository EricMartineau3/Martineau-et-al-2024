function [HbO, HbR] = HemoCompute_HomeMade(DataFolder, SaveFolder, FilterSet, Illumination, b_normalize)
...
if( sum(contains({'red', 'yellow', 'green', 'rx_567'}, lower(Illumination))) < 2 )
...
for indC = 1:size(Illumination,2)
    switch lower(Illumination{indC})
        ...
        case 'yellow'
            ...
            Freq = iYellow.Freq;
        case 'rx_567'
            fidG = fopen([DataFolder 'Rx_567.dat']);
            iGreen = matfile([DataFolder 'Rx_567.mat']);
            NbFrames = min([NbFrames, iGreen.datLength(1,end)]);
            NbPix = iGreen.datSize;
            Freq = iGreen.Freq;
            cTags{2} = 'Rx_567nm';
