function varargout = HemoCorrection_HomeMade(Folder, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% General Infos:
%
...
if AcqInfoStream.MultiCam
    fList = dir([Folder '*.mat']);
    Flag = contains({fList.name},'AcqInfos');
    fList = fList(~Flag);
    for i = 1:length(fList)
        load(fullfile(fList(i).folder,fList(i).name))
        NbFrames(i) = datLength;

    end
    NbFrames = min(min(NbFrames));
end
clearvars -except fList fn Folder idx ind Infos NbFrames tag tmp varargin
...
for ind = 1:size(fList,1)
    ...
    fData = fData(:,1:NbFrames); % Temporary correction to make all the channel at the same size
    fData2 = fData; %fix to avoid dividing by NaN
